...
```python
# Carga directa del archivo dentro de archive/ (.csv.gz) infiriendo el esquema y leyendo encabezados
df = spark.read.options(header='True', inferSchema='True').csv('archive/accepted_2007_to_2018Q4.csv.gz')
...
### Paso 1: Carga, conteo y esquema (Captura 1)
```python
# Carga del archivo dentro de archive/ (.csv.gz) de préstamos rechazados
df = spark.read.options(header='True', inferSchema='True').csv('archive/rejected_2007_to_2018Q4.csv.gz')
